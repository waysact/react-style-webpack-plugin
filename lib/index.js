...
function ReactStylePlugin(filename, options) {
  this.options = options || {};
  this.extractTextPlugin = new ExtractTextPlugin(filename, options);

  if (options.mappingInput) {
    this.styleNameMapping = JSON.parse(fs.readFileSync(options.mappingInput));
  } else {
    this.styleNameMapping = {};
  }
}
...
ReactStylePlugin.prototype.patchEntries = function(compiler) {
  ...
  entries.forEach(function(entry) {
    compiler.options.entry.push(styleEntryLoader(entry), entry);
  });
};
...
ReactStylePlugin.prototype.apply = function(compiler) {
  ...
  compiler.plugin('this-compilation', function(compilation) {
    ...
    compilation.plugin("additional-assets", function(callback) {
      if (self.options.mappingOutput) {
        fs.writeFileSync(
          self.options.mappingOutput,
          JSON.stringify(self.styleNameMapping));
      }
      callback();
    });
  });
